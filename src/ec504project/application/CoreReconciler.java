...
public class CoreReconciler {
	...
	public CoreReconciler(File inputFile, InetAddress inputIP) {
		...
		if (Receiver.Listening(ipAddress)) {
			timer.start();
			isSender = true;
			System.out.println("Connected to IP Address:\t"
					+ ipAddress.toString());
			System.out
					.println("Sender process started, sending file list.\nWaiting for receiver hashes.");
			...
			waiting = false;
			System.out
					.println("Searching for matching blocks and computing reconciliation steps.");
			...
			for (int i = 0; i < receiverHashes.size(); i++) {
				blockMatchers.add(new BlockMatcher(localFileList.fileList
						.get(receiverHashes.get(i).fileIndex).filePath,
						receiverHashes.get(i).hashes,
						receiverHashes.get(i).blockSize));
				sendSteps.add(blockMatchers.get(i).receiverSteps);
			}

			System.out
					.println("Sending reconciliation steps and waiting for ACK.");
			...
			System.out
					.println("Receiver process started.\nWaiting for file list.");
			waiting = true;
			Receiver.Accept();
			timer.start();

			//Receive file list
			ArrayList < FileListElement > senderFileList = Receiver
					.ReceiveList();
			waiting = false;

			System.out
					.println("Comparing file lists and generating hash maps.");

			//Compare file list
			ArrayList < Integer > diffList = localFileList
					.generateDiffList(senderFileList);
			//Generate hashes for non-matching files
			ArrayList < FileSummary > fileSummaries = new ArrayList < FileSummary >();
			ArrayList < SenderData > hashesOnly = new ArrayList < SenderData >(
					diffList.size());
			for (int i = 0; i < diffList.size(); i++) {
				fileSummaries.add(new FileSummary(localFileList.fileList
						.get(diffList.get(i)).filePath));
				hashesOnly.add(fileSummaries.get(i).getSenderData(
						diffList.get(i)));
			}
			//Send hashes
			System.out
					.println("Sending hash maps and waiting for reconciliation steps.");
			Receiver.SendHashes(hashesOnly);
			waiting = true;
			//Reconcile files based on steps
			ArrayList < ArrayList < ReconcileStep >> senderSteps = Receiver
					.ReceiveSteps();
			waiting = false;
			System.out.println("Reconciling files.");
			ArrayList < byte[] > blocks;
			for (int i = 0; i < diffList.size(); i++) {
				blocks = FileSummary.createBlocks(
						localFileList.fileList.get(diffList.get(i)).filePath,
						fileSummaries.get(i).blockSize);
				ReconcileFile.regenerateFile(senderSteps.get(i), blocks,
						localFileList.fileList.get(diffList.get(i)).filePath,
						senderFileList.get(diffList.get(i)).fileHash);
			}
